...
def makePlayerBoard(player, keyGraveyard):
    board_line = []
    if player == thisGame.getActivePlayer():
        board_line.append(sg.Button('Action', key='act', font=('Helvetica', 10)))
        keyCard = "this"
    else:
        keyCard = "other"
    for i in range(4):
        if i < len(player.getBoard()):
            board_line.append(sg.Image(filename= player.getBoard()[i].getImage(), key= f'{keyCard}_{i}'))
        else:
            board_line.append(sg.Image(filename= 'img\Blank_card.png', key= f'{keyCard}_{i}'))
    board_line.append(sg.Button(button_text= 'Graveyard', key= keyGraveyard, font=('Helvetica', 10)))
...
layout = [
    ...
    makePlayerBoard(thisGame.getInactivePlayer(), "other_graveyard"),
    #[sg.Text('Enemey Board', key='other_board', font=('Helvetica', 10)), sg.Button('Graveyard', key='other_graveyard', font=('Helvetica', 10))],
    makePlayerBoard(thisGame.getActivePlayer(), 'this_graveyard'),
